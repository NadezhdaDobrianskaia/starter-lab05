...
class Viewer extends Application {
	...
	$this->data = array_merge($this->data, (array) $this->quotes->get($id));
        $this->caboose->needed('jrating', 'hollywood'); //invoke the rating widget
        $this->data['average'] = ($this->data['vote_count']>0)?($this->data['vote_total']/$this->data['vote_count']):0; //sets average votes
	$this->render();
    }
    
    //handle a rating (to handle the AJAX request from the rating idget)
    function rate(){
